...
use combine::combinator::or;
use combine::primitives::{Error, Info};
use combine::{
    between, eof, many, many1, one_of, optional, parser, sep_by, ParseError, ParseResult, Parser,
    State, Stream,
};
    ...
    I: Stream<Item = char>,
{
    ...
    (sign, or(hex, dec)).map(|(s, x)| s * x).parse_stream(input)
}
